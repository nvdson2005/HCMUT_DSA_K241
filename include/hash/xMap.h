...
V xMap<K, V>::put(K key, V value)
{
    ...
    DLinkedList<Entry *> &currentList = table[index];
    for (auto pEntry : currentList)
    {
...
V xMap<K, V>::remove(K key, void (*deleteKeyInMap)(K))
{
    ...
    for (auto pEntry : currentList)
    {
        if (keyEQ(pEntry->key, key))
        {
            ...
            currentList.removeItem(pEntry, deleteEntry);
            return retValue;
        }
    }
    // key: not found
