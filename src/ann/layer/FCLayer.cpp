...
{
    // YOUR CODE IS HERE
    bool isInTrainingMode = this->m_trainable;
    // If in training mode, cache the input X in m_aCached_X
    // else use X directly
    // Also, if in training mode, add the bias also.
    xt::xarray<double> Y; // output
    if (isInTrainingMode)
    {
        m_aCached_X = X;
    }
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY)
{
    ...
    {
        xt::xarray<double> x = xt::row(m_aCached_X, i);
        xt::xarray<double> Y = xt::row(DY, i);
        xt::xarray<double> dW = xt::linalg::outer(Y, xt::transpose(x));
        xt::view(dW_batch, i) = dW;
    }
    // m_aGrad_W = xt::mean(dW_batch, {0});
    m_aGrad_W = xt::sum(dW_batch, {0});
    m_aGrad_b = DY;
    //m_aGrad_b = xt::mean(m_aGrad_b, {0});
    m_aGrad_b = xt::sum(m_aGrad_b, {0});
    m_unSample_Counter += DY.shape()[0];
    xt::xarray<double> m_aGrad_X;
    if (DY.shape()[0] == 1)
    {
        m_aGrad_X = xt::linalg::dot(xt::transpose(m_aWeights), DY);
    }
    else
    {
        m_aGrad_X = xt::linalg::dot(DY, m_aWeights);
    }
    return m_aGrad_X;
}
