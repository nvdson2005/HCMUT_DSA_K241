...
double_tensor MLPClassifier::predict(double_tensor X, bool make_decision){
    ...
    //DO the inference
    int totalData = X.shape()[0];
    int nClasses = this->get_num_classes();
    //YOUR CODE IS HERE
    xt::xarray<double> Y = xt::zeros<double>({totalData, nClasses});
    Y = this->forward(X);
...
double_tensor MLPClassifier::predict(
    ...
    bool old_mode = this->m_trainable;
    this->set_working_mode(false);
    //Set initial size for results
    int totalData = pLoader->get_sample_count();
    cout << "Total data: " << totalData << endl;
    int nClasses = this->get_num_classes();
    cout << "Number of batches: " << pLoader->get_total_batch() << endl;
    double_tensor results = xt::zeros<double>({totalData, nClasses});
    ...
    unsigned long long total_batch = pLoader->get_total_batch(); 
    unsigned long long batch_idx = 0;
    unsigned long long nsamples = 0;
    for(auto batch: *pLoader){
        //YOUR CODE IS HERE
        double_tensor X = batch.getData();
        double_tensor Y = this->forward(X);
        for(std::size_t i = nsamples; i < X.shape(0) + nsamples; i++){
            if(i == 81) cout << "Predict at 81: " << xt::view(Y, i - nsamples) << endl;
            xt::view(results, i) = xt::view(Y, i - nsamples);
        }
        nsamples += X.shape(0);
    }
...
double_tensor MLPClassifier::evaluate(DataLoader<double, double>* pLoader){
    ...
    for(auto batch : *pLoader){
        ...
        xt::xarray<double> yTrue = batch.getLabel();
        xt::xarray<double> yPred = this->forward(X);
        yPred = xt::argmax(yPred, {1});
        yTrue = xt::argmax(yTrue, {1});
        meter.accumulate(yTrue, yPred);
    }
